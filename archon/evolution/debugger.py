...
class NaturalLanguageDebugger:
    ...
    def _explain_last_failure(self) -> str:
        ...
        lines = [f"📋 Task: {goal}", f"❌ Failed: {error}", "", "Agent Chain:"]
    ...
    def get_forensic_report(self, task_id: str) -> ForensicReport:
        ...
        if not snapshot_row:
            ...
            if not row:
                return ForensicReport(task_id=task_id, goal="", failed_at=0, error="Task not found")

            return ForensicReport(task_id=row[0], goal=row[1], failed_at=row[4] or 0, error=row[3])
        ...
        skill_performance = {s[0]: s[1] / (s[1] + s[2]) if (s[1] + s[2]) > 0 else 0 for s in skills}
