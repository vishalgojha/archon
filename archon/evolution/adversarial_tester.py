    ...
    def _save_result(self, output_id: str, result: TestResult) -> None:
        ...
        conn.execute(
            """INSERT INTO test_results 
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (result.result_id, result.vector_id, output_id, int(result.passed),
             result.severity, result.finding, result.suggestion, result.tested_at)
        )
        conn.commit()
        conn.close()
    ...
    def _generate_report(self, output_id: str, results: list[TestResult]) -> dict[str, Any]:
        ...
        conn.execute(
            """INSERT INTO red_team_reports 
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (report_id, output_id, total, passed, failed, json.dumps(severity_counts),
             score, recommendation, time.time())
        )
        conn.commit()
        conn.close()
